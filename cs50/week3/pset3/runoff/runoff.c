...
bool vote(int voter, int rank, string name)
{
    // finds the index of the candiate
    int ranks = candidate_count-rank-1;
    int index =-1;
    for(int i = 0; i < candidate_count;i++){
...
bool print_winner(void)
{
    ...
    for(int f = 0; f < candidate_count;f++){
        if(candidates[f].votes>ceil(voter_count/2)){
            printf("%s\n", candidates[f].name);
            return true;
        }
    }
...
int find_min(void)
{
    ...
    for(int g = 0; g < candidate_count;g++){
        if(candidates[g].votes < low && candidates[g].eliminated == false){
            low = candidates[g].votes;
        }
    }
    return low;
}
